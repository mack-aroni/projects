...
def forwardElimination(B):
    ...
    for i in range(m - 1):
        ...
        if leftmostNonZeroRow > i:
            ...
            normString += "swap row {} with row {}, ".format(
                i + 1, leftmostNonZeroRow + 1
            )
            latexString += "swap row {} with row {}, ".format(
                i + 1, leftmostNonZeroRow + 1
            )
...
def rowReduce(A, i, j, pivot):
    ...
    # adds each operation to latexString
    normString += "{} * row {} - row {}, ".format(isInt(factor), i + 1, j + 1)
...
def backsubstitution(B):
    ...
    for i in reversed(range(len(C))):
        ...
        if len(r) != 0 and r[0] != len(C[i]) - 1:
            ...
            if isInt(C[i][piv]) != 1:
                latexString += "divide row {} by {}, ".format(
                    i + 1, isInt(C[i][piv]))
                normString += "divide row {} by {}, ".format(
                    i + 1, isInt(C[i][piv]))
            # simplify row
            C[i] = C[i] / C[i][piv]
