...
class Sudoku_Solver:
    ...
    def isSafe(self, val, row, col):
        return not (
            self.row_has_val[row][val]
            or self.col_has_val[col][val]
            or self.subgrid_has_val[row // 3][col // 3][val]
        )
...
if __name__ == "__main__":
    original_grid = np.array(list(str(generators.random_sudoku(avg_rank=150)))).reshape(
        9, 9
    )
